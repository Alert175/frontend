...
const AnchorageInfo = () => {
	...
	// данные по закреплению из новости
	...
	useEffect(() => {
		if (newsData) {
			const { region_indexes, cities_indexes, developers_indexes, complexes_indexes, villages_indexes, url } = newsData;
	...
	useEffect(() => {
		const { result, status } = generateValidUrl("news");
